...
import { reviewAction, queryApproverAction } from "../mask.actions";
import { ColumnData, ReviewDrawer, DetailsDrawer, HistoryDrawer, defaultColumnOrder, defaultColumnsSort, tableConfigColumns, ChangeRequestDrawer, columnConfigColumns, defaultColumnsShown, filterConfigColumns, eligibleDetailsTriggerColumns, rowValueRendererConfigColumns } from "../mask.components";
...
export default function Page() {
	...
	const [columnOrder, setColumnOrder] = useConfigStorage({ localStorageKey: `access-management-mask-${slug}.column-order`, updateIfThisSearhParamExists: "columnOrder", defaultValue: defaultColumnOrderWithActions });
	const [columnsShown, setColumnsShown] = useConfigStorage({ localStorageKey: `access-management-mask-${slug}.columns-shown`, updateIfThisSearhParamExists: "columnsShown", defaultValue: defaultColumnsShownWithActions });
	...
	const query = useQuery({
		queryKey: ["access-management-mask", slug, "approver", {
			keyword,
			filters,
			columnsSort,
			includeDeleted,
				...
				<MenuToolbar
					...
					rightSlot={user.roleMenus.includes(`${slug}#auditor`) ? (
						<div className="flex items-center gap-2">
							<label htmlFor={`access-management-mask-${slug}-approver-show-deleted`} className="text-sm">
								Show Deleted
							</label>
							<Switch
								id={`access-management-mask-${slug}-approver-show-deleted`}
				...
				<ReviewDrawer
					...
					onApprove={() => startMutationTransition(async () => {
						try {
							...
							await reviewAction({ id: reviewDrawerRow!.id, decision: "approve", reviewComment });
							setReviewDrawerOpen(false);
							setReviewComment(lexicalPlainText(""));
							queryClient.invalidateQueries({ queryKey: ["access-management-mask"] });
						} catch(e) { setReviewMutationError(e); }
					})}
					onReject={() => startMutationTransition(async () => {
						try {
							setReviewMutationError(null);
							await reviewAction({ id: reviewDrawerRow!.id, decision: "reject", reviewComment });
							setReviewDrawerOpen(false);
							setReviewComment(lexicalPlainText(""));
							queryClient.invalidateQueries({ queryKey: ["access-management-mask"] });
						} catch(e) { setReviewMutationError(e); }
