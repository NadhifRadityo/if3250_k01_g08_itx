...
export default async function Layout({ children, params }: { children: ReactNode, params: Promise<{ slug: string }> }) {
	...
	const editorMode = thisMenu.modes["mask-editor"];
	if(editorMode != null) return redirect(editorMode.href.replace("/[slug]/", `/${slug}/`));
	const thisMode = thisMenu.modes["mask-viewer"];
