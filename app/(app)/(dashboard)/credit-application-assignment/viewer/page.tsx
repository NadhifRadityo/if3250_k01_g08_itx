...
import { ColumnData, DetailsDrawer, HistoryDrawer, defaultColumnOrder, defaultColumnsSort, tableConfigColumns, ChangeRequestDrawer, ViewerStatistics, columnConfigColumns, defaultColumnsShown, filterConfigColumns, eligibleDetailsTriggerColumns, rowValueRendererConfigColumns } from "../layout.components";

export default function Page() {
	const { user } = useDashboardContext();
	const [keyword, setKeyword] = useState("");
	const [columnOrder, setColumnOrder] = useConfigStorage({ localStorageKey: "credit-application-assignment.column-order", updateIfThisSearhParamExists: "columnOrder", defaultValue: defaultColumnOrder });
	const [columnsShown, setColumnsShown] = useConfigStorage({ localStorageKey: "credit-application-assignment.columns-shown", updateIfThisSearhParamExists: "columnsShown", defaultValue: defaultColumnsShown });
	...
	const query = useQuery({
		queryKey: ["credit-application-assignment", "viewer", {
			keyword,
			filters,
			columnsSort,
			includeDeleted,
	...
	return (
		<MenuPage
			...
			description="View approved and draft credit application assignment requests without edit or review actions."
				...
				<MenuToolbar
					...
					rightSlot={user.roleMenus.includes("credit-application-assignment#auditor") ? (
						<div className="flex items-center gap-2">
							<label htmlFor="credit-application-assignment-viewer-show-deleted" className="text-sm">
								Show Deleted
							</label>
							<Switch
								id="credit-application-assignment-viewer-show-deleted"
