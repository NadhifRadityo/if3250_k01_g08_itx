...
export function MonitoringStatistics({ filters, onFiltersChange }: { filters: MenuFilterState[], onFiltersChange: (v: MenuFilterState[]) => void }) {
	const keys = useStatisticsVisibleKeys({ layoutKey: "message-log.monitoring", cards: commonLogMonitoringCardDefinitions });
	return (
		<StatisticsLoader
			queryKey={["message-log", "monitoring", filters, keys]}
			queryAction={() => uwsa(getMonitoringStatisticsAction)({ filters, keys })}
			refetchInterval={30000}
			render={data => (
				<StatisticsSection layoutKey="message-log.monitoring">
					<CommonLogMonitoringCards data={data} totalLabel="Today's Messages" filters={filters} onFiltersChange={onFiltersChange} />
				</StatisticsSection>
			)}
		/>
	);
...
export function ReportingStatistics({ filters, onFiltersChange }: { filters: MenuFilterState[], onFiltersChange: (v: MenuFilterState[]) => void }) {
	const keys = useStatisticsVisibleKeys({ layoutKey: "message-log.reporting", cards: commonLogReportingCardDefinitions });
	return (
		<StatisticsLoader
			queryKey={["message-log", "reporting", filters, keys]}
			queryAction={() => uwsa(getReportingStatisticsAction)({ filters, keys })}
			render={data => (
				<StatisticsSection layoutKey="message-log.reporting">
					<CommonLogReportingCards data={data} totalLabel="Total Messages" filters={filters} onFiltersChange={onFiltersChange} />
				</StatisticsSection>
			)}
		/>
	);
