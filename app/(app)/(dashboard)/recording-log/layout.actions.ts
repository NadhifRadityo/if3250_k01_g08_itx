...
) => await uwsa(getCommonLogMonitoringStats)({ collectionSlug: "recording-logs", filters, keys }));
export const getReportingStatisticsAction = wsa(async (
	{ filters, keys }: { filters: MenuFilterState[], keys: string[] }
) => await uwsa(getCommonLogReportingStats)({ collectionSlug: "recording-logs", filters, keys }));
