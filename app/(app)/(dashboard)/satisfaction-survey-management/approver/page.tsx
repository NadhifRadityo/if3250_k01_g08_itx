...
import { ColumnData, ReviewDrawer, DetailsDrawer, HistoryDrawer, tableConfigColumns, ChangeRequestDrawer, columnConfigColumns, filterConfigColumns, eligibleDetailsTriggerColumns, rowValueRendererConfigColumns } from "../layout.components";
...
export default function Page() {
	...
	const [columnOrder, setColumnOrder] = useConfigStorage({ localStorageKey: "satisfaction-survey-management.column-order", updateIfThisSearhParamExists: "columnOrder", defaultValue: [] as string[] });
	const [columnsShown, setColumnsShown] = useConfigStorage({ localStorageKey: "satisfaction-survey-management.columns-shown", updateIfThisSearhParamExists: "columnsShown", defaultValue: [] as string[] });
	...
	const [columnsSort, setColumnsSort] = useConfigStorage<[string, boolean][]>({ localStorageKey: "satisfaction-survey-management.columns-sort", updateIfThisSearhParamExists: "columnsSort", defaultValue: [] });
	const [pageIndex, setPageIndex] = useState(1);
	const query = useQuery({
		queryKey: ["satisfaction-survey-management", "approver", {
			keyword,
			filters,
			columnsSort,
			includeDeleted,
	...
	return (
		<MenuPage
			...
			description="Review pending satisfaction survey requests before publication."
		>
				...
				<MenuToolbar
					...
					rightSlot={roles.includes("satisfaction-survey-management-auditor") ? (
						<div className="flex items-center gap-2">
							<label htmlFor="satisfaction-survey-management-approver-show-deleted" className="text-sm">
								Show Deleted
							</label>
							<Switch
								id="satisfaction-survey-management-approver-show-deleted"
