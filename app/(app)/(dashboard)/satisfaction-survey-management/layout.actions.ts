...
) => await uwsa(getCommonReviewableViewerStats)({ collectionSlug: "satisfaction-surveys", filters, keys }));
...
export const getApproverStatisticsAction = wsa(async (
	{ filters, keys }:
	{ filters: MenuFilterState[], keys: string[] }
) => await uwsa(getCommonReviewableApproverStats)({ collectionSlug: "satisfaction-surveys", filters, keys }));
