...
function renderSurveyRequestHistoryValue(
	...
	switch(field) {
		...
		case "description":
			return <ReviewCommentPreview serializedState={value} className="w-full" contentClassName="min-h-9 max-h-44" />;
		case "content":
			return <SurveyContentEditorDialog buttonLabel="View content" dialogTitle="Satisfaction survey history content" disabled value={value} />;
		case "reviewApproved":
...
export function SurveyRequestDetailsDrawer({
	...
	const renderDetailValue = (columnId: SurveyTableColumnId, data: surveyActions.SurveyRequestDetailsOutput) => {
		switch(columnId) {
			...
			case "content":
				return renderDetailColumnValue(columnId, <SurveyContentEditorDialog buttonLabel="View content" dialogTitle="Satisfaction Survey Request Content" disabled value={data.row.content} />);
			case "createdBy":
...
export function useSurveyCellRenderer({ relations, relationNavigation, onOpenRequestChanges }: UseSurveyCellRendererOptions) {
	return useCallback((columnId: SurveyTableColumnId, row: SurveyTableRow) => {
		switch(columnId) {
			...
			case "content":
				return <SurveyContentEditorDialog buttonClassName="w-full" buttonLabel="View content" dialogTitle="Satisfaction Survey Content" disabled value={row.content} />;
			case "createdBy":
