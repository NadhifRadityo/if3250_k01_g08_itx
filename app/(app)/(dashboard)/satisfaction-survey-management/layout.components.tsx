...
import { searchRelationSatisfactionSurveysAction } from "../relation-navigation.actions";
...
export const filterConfigColumns = Object.freeze([
	{ key: "id", label: "Id", type: "relation", relationSearch: uwsa(searchRelationSatisfactionSurveysAction) },
...
export function DetailsDrawer(
	...
	const canAccessHistory = user.roleMenus.includes("satisfaction-survey-management#auditor");
				...
				<div className="flex-1 space-y-2 overflow-y-auto px-4 pb-4">
					{row == null ? (
						<p className="text-muted-foreground text-sm">
							No satisfaction survey request selected.
						</p>
					) : query.isPending ? (
						...
						<Alert variant="destructive">
							...
							<AlertDescription>
								{`${query.error?.message ?? "Unable to load satisfaction survey request details."}`}
							</AlertDescription>
						</Alert>
					) : (
				...
				<div className="flex-1 space-y-3 overflow-y-auto px-4 pb-4">
					{row == null ? (
						...
						<Alert variant="destructive">
							...
							<AlertDescription>You need Satisfaction Survey Management auditor access to view history.</AlertDescription>
						</Alert>
					) : query.isPending ? (
						<div className="space-y-2">
						...
						<p className="text-muted-foreground text-sm">
							No history entries found for this satisfaction survey request.
						</p>
					) : (
...
export function ChangeRequestDrawer(
	...
	const query = useQuery({
		queryKey: ["satisfaction-survey-management", "change-request-diff", row?.id ?? null],
		enabled: open && row != null,
...
export function ViewerStatistics({ filters, onFiltersChange }: { filters: MenuFilterState[], onFiltersChange: (v: MenuFilterState[]) => void }) {
	const keys = useStatisticsVisibleKeys({ layoutKey: "satisfaction-survey-management.viewer", cards: commonReviewableViewerCardDefinitions });
	return (
		<StatisticsLoader
			queryKey={["satisfaction-survey-management", "viewer", filters, keys]}
			queryAction={() => uwsa(getViewerStatisticsAction)({ filters, keys })}
			render={data => (
				<StatisticsSection layoutKey="satisfaction-survey-management.viewer">
					<CommonReviewableViewerCards data={data} totalLabel="Total Satisfaction Surveys" filters={filters} onFiltersChange={onFiltersChange} />
				</StatisticsSection>
			)}
		/>
	);
...
export function ApproverStatistics({ filters, onFiltersChange }: { filters: MenuFilterState[], onFiltersChange: (v: MenuFilterState[]) => void }) {
	const keys = useStatisticsVisibleKeys({ layoutKey: "satisfaction-survey-management.approver", cards: commonReviewableApproverCardDefinitions });
	return (
		<StatisticsLoader
			queryKey={["satisfaction-survey-management", "approver", filters, keys]}
			queryAction={() => uwsa(getApproverStatisticsAction)({ filters, keys })}
			render={data => (
				<StatisticsSection layoutKey="satisfaction-survey-management.approver">
