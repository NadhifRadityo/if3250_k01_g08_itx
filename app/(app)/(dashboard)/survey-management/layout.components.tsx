...
function renderSurveyRequestHistoryValue(
	...
	switch(field) {
		...
		case "description":
			return <ReviewCommentPreview serializedState={value} className="w-full" contentClassName="min-h-9 max-h-44" />;
		case "content":
			return <SurveyContentEditorDialog buttonLabel="View content" dialogTitle="Survey history content" mode="readonly" value={value} />;
		case "reviewApproved":
...
export function SurveyRequestDetailsDrawer({
	...
	const renderDetailValue = (columnId: SurveyTableColumnId, data: surveyActions.SurveyRequestDetailsOutput) => {
		switch(columnId) {
			...
			case "content":
				return renderDetailColumnValue(columnId, <SurveyContentEditorDialog buttonLabel="View content" dialogTitle="Survey Request Content" mode="readonly" value={data.row.content} />);
			case "createdBy":
...
export function useSurveyCellRenderer({ relations, relationNavigation, onOpenRequestChanges }: UseSurveyCellRendererOptions) {
	return useCallback((columnId: SurveyTableColumnId, row: SurveyTableRow) => {
		switch(columnId) {
			...
			case "content":
				return <SurveyContentEditorDialog buttonClassName="w-full" buttonLabel="View content" dialogTitle="Survey Content" mode="readonly" value={row.content} />;
			case "createdBy":
