					...
					<div className="grid gap-3 pb-4 sm:grid-cols-2">
						...
						<div className="space-y-2 sm:col-span-2">
							...
							<SurveyContentEditorDialog
								...
								dialogTitle={formState.surveyId == null ? "Add Survey Content" : "Edit Survey Content"}
								onValueChange={onContentChange}
								value={formState.content}
							/>
							<p className="text-muted-foreground text-xs">Open the editor dialog to update the survey schema content.</p>
						</div>
						{formError != null ? (
...
function renderSurveyRequestReviewDiffValue(field: SurveyRequestReviewDiffField, value: any): ReactNode {
	switch(field) {
		...
		case "content":
			return <SurveyContentEditorDialog buttonLabel="View content" dialogTitle="Review request content" disabled value={value} />;
		case "deletedAt":
...
function renderSurveyRequestHistoryValue(
	...
	switch(field) {
		...
		case "description":
			return <ReviewCommentPreview serializedState={value} className="w-full" contentClassName="min-h-9 max-h-44" />;
		case "content":
			return <SurveyContentEditorDialog buttonLabel="View content" dialogTitle="Survey history content" disabled value={value} />;
		case "reviewApproved":
...
export function SurveyRequestDetailsDrawer({
	...
	const renderDetailValue = (columnId: SurveyTableColumnId, data: surveyActions.SurveyRequestDetailsOutput) => {
		switch(columnId) {
			...
			case "content":
				return renderDetailColumnValue(columnId, <SurveyContentEditorDialog buttonLabel="View content" dialogTitle="Survey Request Content" disabled value={data.row.content} />);
			case "createdBy":
...
export function useSurveyCellRenderer({ relations, relationNavigation, onOpenRequestChanges }: UseSurveyCellRendererOptions) {
	return useCallback((columnId: SurveyTableColumnId, row: SurveyTableRow) => {
		switch(columnId) {
			...
			case "content":
				return <SurveyContentEditorDialog buttonClassName="w-full" buttonLabel="View content" dialogTitle="Survey Content" disabled value={row.content} />;
			case "createdBy":
...
export function useSurveyRequestFilters({ getResolvedFilterColumnConfig }: UseSurveyRequestFiltersOptions): UseSurveyRequestFiltersResult {
	...
	useEffect(() => {
		...
		const pendingNavigation = consumePendingRelationFilterNavigation("survey-management");
