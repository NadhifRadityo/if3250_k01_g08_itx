...
async function attachSurveyTemplates(
	...
	for(const doc of docs) {
		if(typeof doc.surveyVersion == "string" && doc.surveyVersion.length > 0)
			versionIds.add(doc.surveyVersion);
	}
	...
	for(const doc of docs) {
		if(typeof doc.surveyVersion != "string" || doc.surveyVersion.length == 0) continue;
		const template = templates[doc.surveyVersion];
...
function getAnswerEntries(answers: SurveyResult["answers"]): [string, unknown][] {
...
function formatRelation(relations: RelationValues, kind: "users" | "surveys" | "officer-tasks", id: string | null): string {
	if(id == null) return "";
	if(kind == "users") {
...
export async function exportCsvAction(
	...
	for(let i = 0; i < result.docs.length; i++) {
		...
		const row = [
			...
			formatRelation(relations, "surveys", getRelationshipId(doc.survey)),
			doc.surveyVersion ?? "",
	...
	const filename = `survey-results_${mode}_${timestamp}.csv`;
