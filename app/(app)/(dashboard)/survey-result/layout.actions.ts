...
async function resolveRelations(
	...
	Object.assign(relations, await uwsa(resolveRelationSurveys)({ payload, ids: [...surveyIds] }));
...
) => await uwsa(getCommonLogMonitoringStats)({ collectionSlug: "survey-results", filters, keys }));
export const getReportingStatisticsAction = wsa(async (
	{ filters, keys }: { filters: MenuFilterState[], keys: string[] }
) => await uwsa(getCommonLogReportingStats)({ collectionSlug: "survey-results", filters, keys }));
