...
export default function Page() {
	...
	const [columnOrder, setColumnOrder] = useConfigStorage({ localStorageKey: "user-management.column-order", updateIfThisSearhParamExists: "columnOrder", defaultValue: [] as string[] });
	const [columnsShown, setColumnsShown] = useConfigStorage({ localStorageKey: "user-management.columns-shown", updateIfThisSearhParamExists: "columnsShown", defaultValue: [] as string[] });
	...
	const [columnsSort, setColumnsSort] = useConfigStorage<[string, boolean][]>({ localStorageKey: "user-management.columns-sort", updateIfThisSearhParamExists: "columnsSort", defaultValue: [] });
	const [pageIndex, setPageIndex] = useState(1);
	const query = useQuery({
		queryKey: ["user-management", "editor", {
			keyword,
			filters,
			columnsSort,
			includeDeleted,
	...
	return (
		<MenuPage
			...
			description="Manage staged user requests with editor workflows for access, reporting line, and credential changes."
		>
			<RelationNavigationProvider>
				<MenuToolbar
					...
					rightSlot={(
						<>
							{roles.includes("user-management-auditor") ? (
								<div className="flex items-center gap-2">
									<label htmlFor="user-management-editor-show-deleted" className="text-sm">
										Show Deleted
									</label>
									<Switch
										id="user-management-editor-show-deleted"
				...
				<FormDrawer
					...
					onSubmit={() => startMutationTransition(async () => {
						if(editFormDrawerState.email == null || editFormDrawerState.email.trim().length == 0)
							return setEditFormMutationError({ name: "ValidationError", message: "Email is required." });
						...
						if(editFormDrawerState.employeeId == null || editFormDrawerState.employeeId.trim().length == 0)
							return setEditFormMutationError({ name: "ValidationError", message: "Employee ID is required." });
						if(editFormDrawerState.role == null || editFormDrawerState.role.trim().length == 0)
							return setEditFormMutationError({ name: "ValidationError", message: "Role is required." });
						if(editFormDrawerState.initialPassword != null && editFormDrawerState.initialPassword.trim().length > 0 && editFormDrawerState.initialPassword.trim().length < 8)
							return setEditFormMutationError({ name: "ValidationError", message: "Initial password must be at least 8 characters." });
						setEditFormMutationError(null);
						try {
							await requestUpsertAction({
								id: editFormDrawerState.id,
								email: editFormDrawerState.email,
								name: editFormDrawerState.name,
								employeeId: editFormDrawerState.employeeId,
								role: editFormDrawerState.role,
								supervisor: editFormDrawerState.supervisor != null && editFormDrawerState.supervisor.length > 0 ? editFormDrawerState.supervisor : null,
								initialPassword: editFormDrawerState.initialPassword
							});
				...
				<FormDrawer
					...
					onSubmit={() => startMutationTransition(async () => {
						if(addFormDrawerState.email == null || addFormDrawerState.email.trim().length == 0)
							return setAddFormMutationError({ name: "ValidationError", message: "Email is required." });
						...
						if(addFormDrawerState.employeeId == null || addFormDrawerState.employeeId.trim().length == 0)
							return setAddFormMutationError({ name: "ValidationError", message: "Employee ID is required." });
						if(addFormDrawerState.role == null || addFormDrawerState.role.trim().length == 0)
							return setAddFormMutationError({ name: "ValidationError", message: "Role is required." });
						if(addFormDrawerState.initialPassword == null || addFormDrawerState.initialPassword.trim().length < 8)
							return setAddFormMutationError({ name: "ValidationError", message: "Initial password is required for new staged user requests and must be at least 8 characters." });
						setAddFormMutationError(null);
						try {
							await requestUpsertAction({
								email: addFormDrawerState.email,
								name: addFormDrawerState.name,
								employeeId: addFormDrawerState.employeeId,
								role: addFormDrawerState.role,
								supervisor: addFormDrawerState.supervisor != null && addFormDrawerState.supervisor.length > 0 ? addFormDrawerState.supervisor : null,
								initialPassword: addFormDrawerState.initialPassword
							});
