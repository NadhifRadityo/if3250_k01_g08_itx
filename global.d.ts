...
// Payload quick types access
import { GlobalSlug, CollectionSlug, TypedGlobalSelect, TypedCollectionSelect, TransformGlobalWithSelect, TransformCollectionWithSelect } from "payload";
declare const payloadSelectAll: unique symbol;
declare const payloadSelectNone: unique symbol;
declare global {
	type PayloadSelectAll<_Select = null> =
		_Select extends null ?
			typeof payloadSelectAll :
			{
				[_Key in keyof Required<_Select>]:
				Required<_Select>[_Key] extends object ?
					PayloadSelectAll<Required<_Select>[_Key]> :
					true
			};
	type PayloadSelectNone<_Select = null> =
		_Select extends null ?
			typeof payloadSelectNone :
			{
				[_Key in keyof Required<_Select>]:
				Required<_Select>[_Key] extends object ?
					PayloadSelectAll<Required<_Select>[_Key]> :
	...
	type PayloadGlobalSelect<_Slug extends GlobalSlug> = TypedGlobalSelect[_Slug];
	type PayloadCollectionSelect<_Slug extends CollectionSlug> = TypedCollectionSelect[_Slug];
	type PayloadGlobalSelectAll<_Slug extends GlobalSlug> = PayloadSelectAll<PayloadGlobalSelect<_Slug>>;
	type PayloadCollectionSelectAll<_Slug extends CollectionSlug> = PayloadSelectAll<PayloadCollectionSelect<_Slug>>;
	type PayloadGlobalSelectNone<_Slug extends GlobalSlug> = PayloadSelectNone<PayloadGlobalSelect<_Slug>>;
	type PayloadCollectionSelectNone<_Slug extends CollectionSlug> = PayloadSelectNone<PayloadCollectionSelect<_Slug>>;
	type PayloadGlobalType<_Slug extends GlobalSlug, _Select extends PayloadGlobalSelect<_Slug> | PayloadSelectAll | PayloadSelectNone = PayloadSelectAll> =
		_Select extends PayloadSelectAll ?
			TransformGlobalWithSelect<_Slug, PayloadGlobalSelectAll<_Slug>> :
			_Select extends PayloadSelectNone ?
				TransformGlobalWithSelect<_Slug, PayloadGlobalSelectNone<_Slug>> :
				TransformGlobalWithSelect<_Slug, _Select>;
	type PayloadCollectionType<_Slug extends CollectionSlug, _Select extends PayloadCollectionSelect<_Slug> | PayloadSelectAll | PayloadSelectNone = PayloadSelectAll> =
		_Select extends PayloadSelectAll ?
			TransformCollectionWithSelect<_Slug, PayloadCollectionSelectAll<_Slug>> :
			_Select extends PayloadSelectNone ?
				TransformCollectionWithSelect<_Slug, PayloadCollectionSelectNone<_Slug>> :
				TransformCollectionWithSelect<_Slug, _Select>;
	type PayloadUnifiedSlug = `global:${GlobalSlug}` | `collection:${CollectionSlug}`;
	type PayloadUnifiedSlugSelect<_Slug extends PayloadUnifiedSlug> =
		_Slug extends `global:${infer _GlobalSlug}` ?
			PayloadGlobalSelect<_GlobalSlug> :
			_Slug extends `collection:${infer _CollectionSlug}` ?
				PayloadCollectionSelect<_CollectionSlug> :
				never;
	type PayloadType<_Slug extends PayloadUnifiedSlug, _Select extends PayloadUnifiedSlugSelect<_Slug> | PayloadSelectAll | PayloadSelectNone = PayloadSelectAll> =
		_Slug extends `global:${infer _GlobalSlug}` ?
			PayloadGlobalType<_GlobalSlug, _Select> :
			_Slug extends `collection:${infer _CollectionSlug}` ?
				PayloadCollectionType<_CollectionSlug, _Select> :
				never;

	type __PayloadPathsExpandFilter<_Object> =
		Extract<_Object, { id?: string }> extends never ? false : true;
	interface __PayloadPathsExpandWalkJoiner extends ObjectWalkJoiner {
		value1: unknown;
		value2: unknown;
		result: this["value1"] | this["value2"];
	}
	interface __PayloadPathsExpandWalkNext<_Object, _Key, _Previous extends string> extends ObjectWalkNext {
		object: _Object[_Key];
		mapper: __PayloadPathsExpandWalkMapper<_Previous extends null ? ObjectKeyEncode<_Key> : `${_Previous}.${ObjectKeyEncode<_Key>}`>;
		joiner: __PayloadPathsExpandWalkJoiner;
	}
	interface __PayloadPathsExpandWalkMapper<_Previous extends string = null> extends ObjectWalkMapper {
		object: unknown;
		key: unknown;
		result: __PayloadPathsExpandFilter<this["object"][this["key"]]> extends true ?
			_Previous extends null ? ObjectKeyEncode<this["key"]> : `${_Previous}.${ObjectKeyEncode<this["key"]>}` :
			never;
		next: [Extract<this["object"][this["key"]], object>] extends [never] ?
			null :
			__PayloadPathsExpandWalkNext<this["object"], this["key"], _Previous>;
	}
	type PayloadPathsExpand<_Object, _Depth extends number = 10> =
		ObjectWalk<_Object, __PayloadPathsExpandWalkMapper, _Depth>;

	type __PayloadPathsJoinFilter<_Object> =
		Extract<_Object, { docs?: any, hasNextPage?: boolean, totalDocs?: number }> extends never ? false : true;
	interface __PayloadPathsJoinWalkJoiner extends ObjectWalkJoiner {
		value1: unknown;
		value2: unknown;
		result: this["value1"] | this["value2"];
	}
	interface __PayloadPathsJoinWalkNext<_Object, _Key, _Previous extends string> extends ObjectWalkNext {
		object: _Object[_Key];
		mapper: __PayloadPathsJoinWalkMapper<_Previous extends null ? ObjectKeyEncode<_Key> : `${_Previous}.${ObjectKeyEncode<_Key>}`>;
		joiner: __PayloadPathsJoinWalkJoiner;
	}
	interface __PayloadPathsJoinWalkMapper<_Previous extends string = null> extends ObjectWalkMapper {
		object: unknown;
		key: unknown;
		result: __PayloadPathsJoinFilter<this["object"][this["key"]]> extends true ?
			_Previous extends null ? ObjectKeyEncode<this["key"]> : `${_Previous}.${ObjectKeyEncode<this["key"]>}` :
			never;
		next: [Extract<this["object"][this["key"]], object>] extends [never] ?
			null :
			__PayloadPathsJoinWalkNext<this["object"], this["key"], _Previous>;
	}
	type PayloadPathsJoin<_Object, _Depth extends number = 10> =
		ObjectWalk<_Object, __PayloadPathsJoinWalkMapper, _Depth>;

	// Important: The _Path is implicitly sorted from shortest to longest by UnionToTuple.
	type PayloadNeedExpand<_Object, _Path extends PayloadPathsExpand<_Object>> =
		[_Path] extends [never] ?
			_Object :
			UnionToTuple<_Path> extends [infer _First, ...infer _Rest] ?
				PayloadNeedExpand<ObjectPathValueSet<_Object, _First, ObjectPathValueGet<_Object, _First> & object, "required">, _Rest[number]> :
				_Object;
	type PayloadNeedJoin<_Object, _Path extends PayloadPathsJoin<_Object>> =
		[_Path] extends [never] ?
			_Object :
			UnionToTuple<_Path> extends [infer _First, ...infer _Rest] ?
				PayloadNeedJoin<ObjectPathValueSet<_Object, _Path, Required<NonNullable<ObjectPathValueGet<_Object, _Path>>>, "required">, _Rest[number]> :
				_Object;
	type PayloadNeed<
		_Object,
		_Expand extends PayloadPathsExpand<_Object> = never,
		_Join extends PayloadPathsJoin<_Object> = never
	> = PayloadNeedJoin<PayloadNeedExpand<_Object, _Expand>, _Join>;
}
